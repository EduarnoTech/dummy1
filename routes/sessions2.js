...
const Client = require('../models/Clients2');
...
const REFRESH_TOKEN =
  ...
  let vapidKeys = {
    publicKey: 'BMXBU0LFst2ZvBn0kwYmL_30NTSTLDUx7mKwzQc0KCBPVNletGuBEXTn2pxFVHoMebpgj6mRl8DvlLi_J-Dpw-4',
    privateKey: 'YVu4Ix0PWwEYo_pBMTDEhlNz59zdl3BUVwsr4wSUJ3A'
  }
  webPush.setVapidDetails('mailto:shiv7255918@gmail.com',vapidKeys.publicKey,vapidKeys.privateKey)
// add session new
...
router.post('/addSession_old', upload.array('files[]'), async (req, res) =>  {
  ...
  let filelink=[];
    ...
    try {
      ...
      await axios({
        method: 'post',
        url: 'https://device2chat.el.r.appspot.com/api/newSession',
...
router.get('/getSessions/past', async (req, res) => {
  ...
  try {
    const pastSessions = await Session.find({deadline: { $lt : new Date().toISOString() }});
...
router.get('/getSessions/past/:sessionsearch', async (req, res) => {
  ...
  //         path: ['client_id', 'session_id', 'subject', 'type', 'country', 'payment_status', 'work_status', 'assigned_tutors.tutor_id'],
  //         fuzzy: {
  //           prefixLength: 7,
  //         }
  //       },
...
router.get('/getSessions/upcoming', async (req, res) => {
  ...
  try {
    const upcomingSessions = await Session.find({deadline: { $gte : new Date(tomorrow).toISOString() }});
...
router.get('/getSessions/today', async (req, res) => {
  ...
  try {
    const todaySessions = await Session.find({deadline: { $gte : new Date(today).toISOString(), $lt: new Date(tomorrow).toISOString() }});
...
router.post('/filter/filterSessions', async (req, res) => {
  ...
  try {
    ...
    if(req.body.work_status && req.body.payment_status && req.body.from && req.body.to && req.body.tutor_payment_status) {
    ...
    } else if(!req.body.work_status && req.body.payment_status && req.body.from && req.body.to && req.body.tutor_payment_status) {
      sessions = await Session.find({
        payment_status: req.body.payment_status,
        tutor_payment_status: req.body.tutor_payment_status
      });
    ...
    } else if(req.body.work_status && !req.body.payment_status && req.body.tutor_payment_status && req.body.from && req.body.to) {
      sessions = await Session.find({
        work_status: req.body.work_status,
        tutor_payment_status: req.body.tutor_payment_status
      });
    ...
    res
      .status(200)
      .json({ length: filteredSessions.length, result: sortedFilteredSessions });
...
router.post('/subscriber', async (req, res) => {
  try {
    console.log("entered in subscriber api")
    const subscription=req.body;
    // const session = await Session.findOneAndUpdate(
    //   { session_id: req.body.session_id },
    //   {
    ...
    const Payload =JSON.stringify({title:"New Task"});
  ...
  try {
    ...
    if (result1) {
      if (
        ...
        !result1?.notified_tutors.find((el) => el.tutor_id == tutorId)?.cancel_tutor
      ) {
        if (!result1.tutor_interested) {
          ...
          let tutor_email=result1?.tutor_interested?.find((el1) => el1.tutor_id == tutorId)?.email;
        ...
        let clientId=result1.client_id;
        let work_status=result1.work_status;
        res.status(200).json({ success: false, status: 'cancelled',tutor_email:tutor_email,work_status:work_status,clientId:clientId });
      } else {
...
router.put('/cancelNotifiedTutor/:sessionId/:tutorId', async (req, res) => {
  try {
    const updateTutor = await Session.updateOne(
      {session_id: req.params.sessionId, "notified_tutors.tutor_id": req.params.tutorId},     
...
router.post('/storeTutorResponse', async (req, res) => {
  ...
  try {
    ...
    await axios({
      method: 'post',
      url: 'https://device2chat.el.r.appspot.com/api/interestedTutors',
...
router.put('/assigned-tutor-acceptance-status/:sessionId/:tutorId', async (req, res) => {
  try {
    const updateTutor = await Session.updateOne(
      {session_id: req.params.sessionId, "assigned_tutors.tutor_id": req.params.tutorId},     
...
router.put('/update-tutor-payment-status/:sessionId/:waId/:poutId', async (req, res) => {
  try {
...
router.put('/tutor-session-rating/:tutorId/:sessionId', async (req, res) => {
  ...
  try {
    ...
    // });
    const tutor = await Session.findOne({session_id: req.params.sessionId, "assigned_tutors.tutor_id": req.params.tutorId});
    if ( tutor.assigned_tutors.filter((el2)=>el2.tutor_id===req.params.tutorId)[0].rating &&
    tutor.assigned_tutors.filter((el2)=>el2.tutor_id===req.params.tutorId)[0].rating.length !== 0) newRating = (+newRating + +tutor.assigned_tutors.filter((el2)=>el2.tutor_id===req.params.tutorId)[0].rating) / 2;
    ...
    if(!tutor?.assigned_tutors?.filter(el=>el.tutor_id===req.params.tutorId)[0]?.rating_and_reviews)
    {
     ...
     if(updateTutorRating){
      const updatetut = await Session.updateOne(
        {session_id: req.params.sessionId, "assigned_tutors.tutor_id": req.params.tutorId},     
        {
          $set:{"assigned_tutors.$.session_rating" : req.body.session_rating ,"assigned_tutors.$.rating":""+newRating},
          $push:{"assigned_tutors.$.rating_and_reviews":ratingObj, "assigned_tutors.$.sessions_done": req.body.session_done}
         
        });
        res.status(200).json({session: updatetut,success:"New array field"})
     }else{
       res.status(400).json("rating_and_review field error!")
     }
     
    }else if(tutor?.assigned_tutors?.filter(el=>el.tutor_id===req.params.tutorId)[0]?.rating_and_reviews?.filter(el1=>el1.sessionId===req.body.session_done)?.length!==0)
    {
      const updateTutorRating = await Session.updateOne(
      {session_id: req.params.sessionId, "assigned_tutors.tutor_id": req.params.tutorId},     
      { $set: { 
...
router.put('/set-tutor-deadline', async (req, res) => {
  try {
    const updateTutorDeadline = await Session.updateOne(
      {session_id: req.body.sessionId},
      { $set: { tutor_deadline: req.body.tutor_deadline, tutor_duration: req.body.tutor_duration } }
    );
...
router.post('/validateWaNumber', async (req, res) => {
  ...
  try {
    ...
    if (result1) {
      if (
        result1?.notified_tutors &&
        result1.notified_tutors.find((el) => el.tutor_id == tutorId && el.wa_id==waNumber) 
      ) {
...
router.put('/assignedTutorUpdateAmount', async (req, res) => {
  try {
    const session_tutorAmt = await Session.updateOne(
      {session_id: req.body.session_id, "assigned_tutors.tutor_id": req.body.tutor_id},     
      {
        $set: { "assigned_tutors.$.tutor_dealt_amount" : req.body.tutor_dealt_amount },
      },{
        upsert:true,new: true
     });
    res.status(200).json({ success: true, result: session_tutorAmt });
...
router.post('/tutorEmailNotification', async (req, res) => {
  try {
     ...
     let transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: 'tutorpointdevice2@gmail.com',
        pass: 'izexygajptfbkgrn'
      }
    });
    
    let mailOptions = {
      from: 'tutorpointdevice2@gmail.com',
      to: req.body.email,
      subject: `${req.body.param1} | ${req.body.param3}`,
      html: `<center><img src="https://tutorpoint.in/assets/images/logo.png" style="width: 125px" /></center><br /><br /><p><b>Dear ${req.body.name},</b></p><p>We have a session for you.</p><p>Session ID: ${req.body.param2}</p><p>Subject/Topic name: ${req.body.param3}</p><p>Date and Time ${req.body.templateName.includes('live_session_tutor_notify') ? '' : '(Deadline)'}: ${req.body.param4}</p><p>Duration: ${req.body.templateName.includes('live_session_tutor_notify') ? req.body.param5 : 'N/A'}</p><p>We will pay you ${req.body.param6}</p><p>Are you confident in this subject? Can you perform very well in this session? If yes, then press on the below button to show your interest. Please check the study materials after you press the button below.</p><button style='background-color: #59C173; padding: 10px 18px; border: 1px solid #59C173; border-radius: 20px'><a href="https://tutor-response.tutorpoint.in/${req.body.param7}" target="_blank" style='text-decoration: none; color: #ffff'>I am Interested</a></button><p>Please wait after you show your interest. We will get back to you shortly to get your confirmation for this session. 
      Disclaimer:</p><p><b>1.Never take these sessions casually. They impact your ratings.</b></p><p><b>2. Showing interest doesn't mean that we have assigned the session to you.</b></p><p>To stop receiving Whatsapp notifications from us, send an official email to us.</p><br /><p>Thanks,</p><p>Team Tutorpoint</p><p>Email: support@tutorpoint.in, Contact: +917091489698</p><br/><br /><footer><p>Copyright © 2020 Tutorpoint. All rights reserved Abhay Education Pvt. Ltd.</p></footer>`
    };
