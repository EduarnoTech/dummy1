...
const Session = require("../models/Sessions6");
const Client = require("../models/Clients6");
...
router.post("/addSession_old", upload.array("files[]"), async (req, res) => {
  ...
  try {
    ...
    await axios({
      method: "post",
      url: "https://device6chat.el.r.appspot.com/api/newSession",
...
router.post("/storeTutorResponse", async (req, res) => {
  ...
  try {
    ...
    await axios({
      method: "post",
      url: "https://device6chat.el.r.appspot.com/api/interestedTutors",
...
router.post("/tutorEmailNotification", async (req, res) => {
  try {
    ...
    let transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: "tutorpoint.d6.official@gmail.com",
        pass: "wyfrledyiesgnihp",
      },
    });

    let mailOptions = {
      from: "tutorpoint.d6.official@gmail.com",
      to: req.body.email,
      ...
      Disclaimer:</p><p><b>1.Never take these sessions casually. They impact your ratings.</b></p><p><b>2. Showing interest doesn't mean that we have assigned the session to you.</b></p><p>To stop receiving Whatsapp notifications from us, send an official email to us.</p><br /><p>Thanks,</p><p>Team Tutorpoint</p><p>Email: support@tutorpoint.in, Contact: +917761814102</p><br/><br /><footer><p>Copyright © 2020 Tutorpoint. All rights reserved Abhay Education Pvt. Ltd.</p></footer>`,
    };
...
try {
  const result = await Session.find({ assigned_tutors: {$elemMatch:{tutor_id:tutorId } } });
  result.map((el1)=> {
    // console.log({el1})
    el1?.assigned_tutors?.forEach((el2)=>{
      ...
      //  el2.pout_info.map((el3)=>{
        // console.log({el3})
        filterArr(el2.pout_info.sort((a, b) => {return b.created_at - a.created_at}), (it) => (it.id)).map((i)=>{
          // console.log({i})
          // i.map((el5)=>{
