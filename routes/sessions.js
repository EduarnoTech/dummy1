      ...
      $search: {
        wildcard: {
          ...
          path: [
            ...
            { wildcard: "type*" },
            { wildcard: "country*" },
            { wildcard: "payment_status*" },
            { wildcard: "work_status*" },
            { wildcard: "assigned_tutors.tutor_id*" },
            { wildcard: "payment_info.id*" },
          ],
...
router.post("/storeTutorResponse", async (req, res) => {
  ...
  try {
    ...
    await axios({
      method: "post",
      url: "https://eduarnochatapp.el.r.appspot.com/api/interestedTutors",
  ...
  try {
    ...
    if (
      tutor.assigned_tutors.filter(
        (el2) => el2.tutor_id === req.params.tutorId
      )[0].rating &&
      tutor.assigned_tutors.filter(
        (el2) => el2.tutor_id === req.params.tutorId
      )[0].rating.length !== 0
...
router.post("/tutorEmailNotification", async (req, res) => {
  try {
    ...
    let transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: "tutorpointdevice1@gmail.com",
...
//send email notification to tutors about TDS Certi
router.post("/tutorEmailNotificationToAll", async (req, res) => {
  const oauth2client = new google.auth.OAuth2(
    CLIENT_ID,
    CLIENT_SECRET,
    REDIRECT_URI
  ...
  try {
    const tut1 = await Tutor.find({});
    //   const tut1 =[{
    //     email:"shiv7255918@gmail.com",
    //     pan:"KKCPK4165C"
    //   },
    // {
    //   email:"tech.eduarno@gmail.com",
    //   pan:"KIQPS5802E"
    // },
    // {
    //   email:"eduarno.tech@gmail.com",
    //   pan:"KDSPS0619N"
    // },
    // {
    //   email:"studybuddy365@gmail.com",
    //   pan:"JYXPS8931Q"
    // }]
    ...
    var clientFolderSearch = await drive?.files?.list({
      q: `mimeType='application/vnd.google-apps.folder' and name='TDS'`,
      fields: "files(id,name)",
    });

    if (clientFolderSearch?.data?.files?.length !== 0) {
      var CertificatesQ2 = await drive.files.list({
        q: ` name='Certificates Q2' and '${clientFolderSearch?.data?.files[0].id}' in parents`,
        fields: "files(id,name)",
      });
      // console.log({ sessionisthere: sessionFolderSearch.data.files });

      if (CertificatesQ2?.data?.files?.length !== 0) {
        var CertificatesAr = await drive.files.list({
          q: ` name='Certificates' and '${CertificatesQ2?.data?.files[0].id}' in parents`,
          fields: "files(id,name)",
        });

        if (CertificatesAr.data.files.length != 0) {
          var certifile = await drive.files.list({
            q: `'${CertificatesAr?.data?.files[0].id}' in parents`,
            fields: "files(id,name)",
          });
          const certificateQ2Ar = certifile.data.files;
          certificateQ2Ar.map(async (certi) => {
            tutAr.map(async (tut) => {
              try {
                if (certi.name.split("_", 1)[0] === tut.pan) {
                  await drive.permissions.create({
                    fileId: certi.id,
                    requestBody: {
                      role: "reader",
    ...
    res.status(200).json({ tdsAr, tutAr });
  } catch (err) {
    console.log(err);
    res.status(500).json("Email not sent");
  }
...
//send mail notification regarding password filling
router.post("/tutorSetPassword_tutorlancer", async (req, res) => {
  try {
    // sending assigned session mail to tutor
    let transporter = nodemailer.createTransport({
      service: "gmail",
...
// get payment info of tutors from web
router.post("/payment_details", async (req, res) => {
  ...
  try {
    const result = await Session.find({
      assigned_tutors: { $elemMatch: { tutor_id: tutorId } },
    });
    result.map((el1) => {
      // console.log({el1})
      el1?.assigned_tutors?.forEach((el2) => {
        if (el2.tutor_id == tutorId) {
          sessionId = el1.session_id;
          type = el1.type;
          subject = el1.subject;

          //  el2.pout_info.map((el3)=>{
          // console.log({el3})
          filterArr(
            el2.pout_info.sort((a, b) => {
            ...
            //   paymentI.transaction_id=el5.id;
            //   paymentI.session_id=el5.session_id;
            //   paymentI.date=el5.created_at;
            //   // paymentI.transaction_id=i.id;
            //   paymentI.status=el5.status;
            //   paymentI.amount=el5.amount;
            // })
            console.log({ created_at: i.created_at });
            i.session_id = sessionId;
            i.description = subject + "-" + type;
            date1 = new Date(+i.created_at);
            let date2 = moment(date1).format("lll");
            i.created_at = date2;
            let am = +i.amount / 100;
            i.amount = i.currency + " " + am;
            // paymentI.transaction_id=i.id;
            // paymentI.session_id=i.session_id;
            // paymentI.date=i.created_at;
            // // paymentI.transaction_id=i.id;
            // paymentI.status=i.status;
            // paymentI.amount=i.amount;

            paymentInfo.push(i);
          });

          //  })
          // paymentInfo.push(el2)
        }
      });
    });
    ...
    //   paymentI.transaction_id=paymentInfo[j].id;
    //   paymentI.session_id=paymentInfo[j].session_id;
    //   paymentI.date=new Date(1639644642);
    //   paymentI.description=paymentInfo[j].session_id;
    //   paymentI.amount=paymentInfo[j].amount;
    //   paymentI.status=paymentInfo[j].status;
    //   payment_submit.push(paymentI)
    // }

    // console.log('result:', result);
    console.log("paymentInfo:", paymentInfo);
    // console.log('payment_submit:', payment_submit);
    console.log({ tuuutooor_id: tutorId });
    res.status(200).json({ success: true, result: paymentInfo });

    console.log("tutor payment details");
  } catch (err) {
    console.log(err);
    res.status(500).json(err);
    console.log("not_working tutor payment details");
  }
});

// // fetch session information of a tutor
// router.post("/private/fetchTutorSession", async (req, res) => {
//   try {
//     const tutor_id = req.body.tutor_id;
//     let resultArray =[];
//     let sessionAr = [];
//     let upcomingSessionAr = [];
//     let allSessions = [];

//     const result = await Session.find({
//       assigned_tutors: { $elemMatch: { tutor_id: tutor_id ,acceptance_status : "accepted"} },
//     });
//     // resultArray=[...resultArray,...result]

//     const result2 = await Session2.find({
//       assigned_tutors: { $elemMatch: { tutor_id: tutor_id ,acceptance_status : "accepted"} },
//     });

//     const result3 = await Session3.find({
//       assigned_tutors: { $elemMatch: { tutor_id: tutor_id ,acceptance_status : "accepted"} },
//     });

//     const result4 = await Session4.find({
//       assigned_tutors: { $elemMatch: { tutor_id: tutor_id ,acceptance_status : "accepted"} },
//     });

//     const result5 = await Session5.find({
//       assigned_tutors: { $elemMatch: { tutor_id: tutor_id ,acceptance_status : "accepted"} },
//     });

//     resultArray=[...result,...result2,...result3,...result4,...result5]

//     console.log({ resultArray });
//     if (result) {
//       // console.log({result})
//       resultArray?.map((el) => {
//         const assignedTut = el?.assigned_tutors?.filter(
//           (el2) => el2?.tutor_id === tutor_id && el2?.acceptance_status === "accepted"
//         );
//         const proccessedPayouts = assignedTut[0]?.pout_info?.filter(
//           (el2) => el2?.status === "processed"
//         );
//         console.log({ proccessedPayouts, assignedTut });

//         const cur = proccessedPayouts && proccessedPayouts[0]?.currency;
//         const amount = proccessedPayouts && proccessedPayouts[0]?.amount;
//         console.log({ cur, amount });
//         const deadL = el.tutor_deadline ? el.tutor_deadline : el.deadline;
//         const dur = el.tutor_duration ? el.tutor_duration : el.duration;
...
//           deadline: deadL,
//           duration: dur,
//           // client_time:el.tutor_deadline ? el.tutor_deadline:el.deadline ,
//           client_id:el.client_id,
//           work_status: el.work_status,
//           payment_status: el.tutor_payment_status,
//           client_comments: el.client_comments,
//           client_amount: amount,
//           tutor_id: tutor_id,
//           currency: cur ? cur : "",
//           // rating_client:el.rating_client,
//           quesFolderLink: el.quesFolderLink,
//         };
//         const date_time41 = moment(deadL).format("MM/DD/YYYY hh:mm:ss A");

//         let difference = +new Date(date_time41) - +Date.now();
//         console.log({ differneceTest: difference });

//         if (difference <= 0) sessionAr.push(tempObj);
//         else {
//           upcomingSessionAr.push(tempObj);
//         }

//         allSessions.push(tempObj);
//       });

//       res.status(200).send({
//         sessionData: sessionAr,
//         upcomingSessionData: upcomingSessionAr,
//         allSessions: allSessions,
//       });
//       console.log("session data sent");
//     } else {
//       res.status(400).send("Not able to send session data!");
//     }
//     // }
//   } catch (err) {
//     console.log(err);
...
// // fetch session information of a tutor
// router.post("/private/fetchNotifiedSession", async (req, res) => {
//   try {
//     const tutor_id = req.body.tutor_id;
//     let notifiedAr = [];
//     let interestedAr = [];
...
//     // device1
//     const result = await Session.find({
//   notified_tutors: { $elemMatch: { tutor_id: tutor_id }}
//       // {assigned_tutors:{$and: [{$elemMatch: { tutor_id: tutor_id}} ,{$elemMatch:{"acceptance_status": {$ne:"accepted"}}}]}}]
//     });

//     const result1 = await Session.find({
//       assigned_tutors: { $elemMatch: { tutor_id: tutor_id, acceptance_status: "accepted" }}
//         // {assigned_tutors:{ $elemMatch: { tutor_id: tutor_id} ,$not : {$elemMatch:{acceptance_status:"accepted"}}}}
//       });

//       result.map((el,index)=>{
//         if (result1.filter(el2=>el2.tutor_id===el.tutor_id)){
...
//           const result2_1 = await Session2.find({
//             assigned_tutors: { $elemMatch: { tutor_id: tutor_id, acceptance_status: "accepted" }}
//               // {assigned_tutors:{ $elemMatch: { tutor_id: tutor_id} ,$not : {$elemMatch:{acceptance_status:"accepted"}}}}
//             });

//             result2.map((el,index)=>{
//               if (result2_1.filter(el2=>el2.tutor_id===el.tutor_id)){
//                 result2.splice(index, 1)
//               }
//             })

//             resultArray=[...resultArray,...result2]
...
//                 const result3_1 = await Session3.find({
//                   assigned_tutors: { $elemMatch: { tutor_id: tutor_id, acceptance_status: "accepted" }}
//                     // {assigned_tutors:{ $elemMatch: { tutor_id: tutor_id} ,$not : {$elemMatch:{acceptance_status:"accepted"}}}}
//                   });

//                   result3.map((el,index)=>{
//                     if (result3_1.filter(el2=>el2.tutor_id===el.tutor_id)){
//                       result3.splice(index, 1)
//                     }
//                   })

//                   resultArray=[...resultArray,...result3]
...
//                       const result4_1 = await Session4.find({
//                         assigned_tutors: { $elemMatch: { tutor_id: tutor_id, acceptance_status: "accepted" }}
//                           // {assigned_tutors:{ $elemMatch: { tutor_id: tutor_id} ,$not : {$elemMatch:{acceptance_status:"accepted"}}}}
//                         });

//                         result.map((el,index)=>{
//                           if (result4_1.filter(el2=>el2.tutor_id===el.tutor_id)){
//                             result4.splice(index, 1)
//                           }
//                         })

//                         resultArray=[...resultArray,...result4]
...
//                             const result5_1 = await Session.find({
//                               assigned_tutors: { $elemMatch: { tutor_id: tutor_id, acceptance_status: "accepted" }}
//                                 // {assigned_tutors:{ $elemMatch: { tutor_id: tutor_id} ,$not : {$elemMatch:{acceptance_status:"accepted"}}}}
//                               });

//                               result5.map((el,index)=>{
//                                 if (result5_1.filter(el2=>el2.tutor_id===el.tutor_id)){
//                                   result5.splice(index, 1)
//                                 }
//                               })

//                               resultArray=[...resultArray,...result5]

//           console.log({ resultTEST11:result2 });

//     console.log({ resultTEST:resultArray });
//     if (result  ) {
//       // console.log({result})
//       resultArray?.map(async(el) => {

//         const deadL = el.tutor_deadline ? el.tutor_deadline : el.deadline;
//         const dur = el.tutor_duration ? el.tutor_duration : el.duration;

//        let showedInterest =el?.tutor_interested?.filter(el2=>el2.tutor_id===tutor_id);
//        let acceptRequest =el?.assigned_tutors?.filter(el2=>el2.tutor_id===tutor_id);

//        if(showedInterest.length!==0){
//         // const cur = proccessedPayouts && proccessedPayouts[0]?.currency;
//         // const amount = proccessedPayouts && proccessedPayouts[0]?.amount;
//         // console.log({ cur, amount });
...
//           quesFolderLink: el.quesFolderLink,
//           showedInterest: true,
//           acceptRequest: acceptRequest.length !==0 ? true : false
//         };
//         notifiedAr.push(tempObj)
//       }else{
//         let tempObj = {
//           sessionId: el.session_id,
//           type: el.type,
